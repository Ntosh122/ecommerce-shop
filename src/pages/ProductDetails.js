...
const ProductDetails = () => {
  // recieving the product id from the url
  const {id} = useParams();
  const {products} = useContext(ProductContext)
  const {addToCart} = useContext(CartContext)

  // getting a single product based on the id of the product
  const product = products.find((item) =>{
    return item.id === parseInt(id)
  })
  // if the product is not found in the shopping list
  if(!product){
    return (
      ...
      <div className='container mx-auto lg:mx-0'>
        <div className='flex flex-col lg:flex-row items-center'>
          ...
          <div className='flex-1 text-center lg:text-left'>
            <h1 className='text-[26px] font-medium mb-2 max-w-[450px] auto'>{title}</h1>
